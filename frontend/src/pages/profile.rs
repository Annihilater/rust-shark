        ...
        <Layout>
            <div class="max-w-3xl mx-auto space-y-6">
                ...
                {move || (!error.get().is_empty()).then(|| view! {
                    <div class="notice-error rounded-lg p-3 text-sm">
                        {error.get()}
                    </div>
                })}
                {move || (!success.get().is_empty()).then(|| view! {
                    <div class="bg-green-50 border border-green-200 text-green-700 dark:bg-green-900/50 dark:border-green-700 dark:text-green-300 rounded-lg p-3 text-sm">
                        {success.get()}
                    </div>
                })}
                ...
                <div class="card rounded-xl p-5 space-y-4">
                    <div class="flex items-center justify-between">
                        ...
                        {move || if !edit_mode.get() {
                            view! {
                                <button
                                    class="text-sm bg-blue-600 hover:bg-blue-500 text-white px-3 py-1 rounded-md transition-colors"
                            ...
                            view! {
                                <div class="flex gap-2">
                                    <button
                                        class="text-sm bg-green-600 hover:bg-green-500 text-white px-3 py-1 rounded-md transition-colors"
                                        on:click=save_profile
                                    >
                                        "保存"
                                    </button>
                                    <button
                                        class="text-sm btn-secondary px-3 py-1 rounded-md transition-colors"
                            ...
                            <div>
                                ...
                                {if edit_mode.get() {
                                    view! {
                                        <input
                                            type="text"
                                            class="input w-full rounded-md px-3 py-1.5 text-sm text-gray-900 dark:text-white"
                                    ...
                                    view! {
                                        <p class="text-sm">{p.name.clone()}</p>
                                    }.into_any()
                                }}
                            </div>
                            <div>
                                ...
                                {if edit_mode.get() {
                                    ...
                                    view! {
                                        <p class="text-sm">{p.email.clone()}</p>
                                    }.into_any()
                                }}
                            </div>
                            <div>
                                ...
                                <p class="text-sm">
                                    {if p.role == "admin" {
                                        view! { <span class="text-yellow-500 dark:text-yellow-400 font-medium">"管理员"</span> }.into_any()
                                    } else {
                                        view! { <span class="text-gray-600 dark:text-gray-300">"普通用户"</span> }.into_any()
                                    }}
                                </p>
                            </div>
                            <div>
                                <label class="block text-xs text-gray-500 dark:text-gray-400 mb-1">"注册时间"</label>
                                <p class="text-sm text-gray-600 dark:text-gray-300">{p.created_at.clone()}</p>
                            </div>
                        </div>
                    })}
                </div>
                ...
                <div class="card rounded-xl p-5 space-y-4">
                    ...
                    {move || show_password_form.get().then(|| view! {
                        <div class="space-y-3 max-w-sm">
                            <div>
                                <label class="block text-xs text-gray-500 dark:text-gray-400 mb-1">"原密码"</label>
                                <input
                                    type="password"
                                    class="input w-full rounded-md px-3 py-1.5 text-sm text-gray-900 dark:text-white"
                            ...
                            <div class="flex gap-2 pt-1">
                                ...
                                <button
                                    class="btn-secondary px-4 py-1.5 rounded-md text-sm transition-colors"
                                    ...
                                    <table class="w-full text-sm">
                                        ...
                                        <tbody>
                                            {logs.into_iter().map(|log| view! {
                                                <tr class="table-row">
                                                    <td class="py-2 pr-4 text-gray-600 dark:text-gray-300 whitespace-nowrap">{log.created_at}</td>
                                                    <td class="py-2 pr-4 text-blue-600 dark:text-blue-300 font-mono">{log.ip}</td>
                                                    <td class="py-2 text-gray-500 text-xs max-w-xs truncate">{log.user_agent}</td>
                                                </tr>
                                            }).collect_view()}
                                        </tbody>
                                    </table>
